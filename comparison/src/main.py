...
distributions = ['gaussian']  # Can be 'gaussian', 'student' or 'dirichlet'
structures = ['asia'] # Structures can be added in otagrum-experiments/comparison/data/structures


size_min = {'asia':100, 'alarm':100}  # Domain for strucural results
size_max = {'asia':1000, 'alarm':15000}
n_points = {'asia':10, 'alarm': 15} # Number of points for which results are computed
n_restart = {'asia': 1, 'alarm':5} # Number of restarts for averagins results

xlim = {'asia':4000, 'alarm':6000} # Domain for plotting figures
    ...
    for distribution in distributions:
        ...
        if distribution == 'gaussian' or distribution == 'student':
            for correlation in correlations:
                ...
                plt.savefig(os.path.join(apath+'/scores',
                                         '_'.join(['fscore', distribution , structure]) +'.pdf'),
                            transparent=True)
                print("Saving figure in {}".format(os.path.join(apath+'/scores',
                                                                '_'.join(['fscore', distribution , structure])+'.pdf'),
                                                                transparent=True))
                ...
                # cbic_bernstein.plotScore('hamming', fig, ax, **plot_style_cbic_bernstein)
                plt.savefig(os.path.join(apath+'/times',
                                         '_'.join(['time_complexity', distribution , structure]) +'.pdf'),
                        transparent=True)
                print("Saving figure in {}".format(os.path.join(apath+'/times',
                                                   '_'.join(['time_complexity', distribution , structure]) +'.pdf'),
                                                   transparent=True))
            ...
            ax.legend(loc='lower right')
            # cbic_bernstein.plotScore('skelF', fig, ax, **plot_style_cbic_bernstein)
            plt.savefig(os.path.join(bpath+'/scores',
                                     '_'.join(['fscore', distribution , structure]) +'.pdf'),
                        transparent=True)
            ...
            # cbic_gaussian.plotTime(fig, ax, **plot_style_cbic_gaussian)
            # cbic_bernstein.plotScore('hamming', fig, ax, **plot_style_cbic_bernstein)
            plt.savefig(os.path.join(bpath+'/times',
                                     '_'.join(['time_complexity', distribution , structure]) +'.pdf'),
                        transparent=True)
